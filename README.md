...
MLOps Engineer who architects systems, not just deploys them. Built a production multi-cloud ML platform from scratch (GKE + EKS), documenting **17 Architectural Decision Records** with measured trade-offs — the same engineering discipline used by mid-to-senior platform teams.

What I do differently: I diagnose production incidents from first principles and document the reasoning, not just the fix.

- **AWS Certified Machine Learning — Specialty** | TripleTen Data Science Graduate
- Open to **MLOps / ML Infrastructure / ML Platform** roles (Remote preferred)
...
**What makes it different — incidents diagnosed, not just tools listed:**
- **81% error rate → 0%**: Diagnosed `uvicorn --workers N` anti-pattern under K8s (shared CPU budget → thrashing). Resolved via async inference + ThreadPoolExecutor exploiting GIL release in sklearn C extensions (ADR-014/015)
- **HPA never scales down**: Proved mathematically that memory-based HPA fails for ML services (fixed RAM footprint). CPU-only HPA achieved correct 3→1 scale-down (ADR-001)
- **SHAP all-zero bug**: Traced to TreeExplainer incompatibility with StackingClassifier. Implemented KernelExplainer in original 10-feature space for business interpretability (ADR-010)
- **Data leakage fix**: Found same-period aggregates in ChicagoTaxi. Replaced with lag features + temporal split. R² 0.905 → 0.965 (ADR-009)
- **17 ADRs**, 395+ tests (90–98% cov), Terraform multi-cloud IaC, 10-job CI/CD matrix, daily drift detection, Prometheus + Grafana (26 panels)
...
## Selected Engineering Decisions ([all 17 ADRs](https://duqueom.github.io/ML-MLOps-Portfolio/architecture/decisions/))

| ADR | Decision | The Harder Choice |
|-----|----------|-------------------|
| 001 | CPU-only HPA | Proved memory HPA mathematically fails for ML |
| 007 | No Feature Store | Designed Feast architecture for when it IS needed |
| 009 | Simplification | Removed a project because MAPE 32.9% isn't defensible |
| 010 | SHAP KernelExplainer | Diagnosed production bug, evaluated 4 alternatives |
| 014/015 | Single-worker + async | Found uvicorn anti-pattern → 81% errors → 0% |
| 016 | GCP/AWS parity | Chose $24/mo over $145/mo, documented the SLA math |
...
*"I don't just deploy models — I diagnose why they fail and document the reasoning."*
